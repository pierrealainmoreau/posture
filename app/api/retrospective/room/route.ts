...
export async function POST(req: NextRequest) {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    if (playerErr || !player) {
      await admin.from("retro_rooms").delete().eq("id", room.id);
      return NextResponse.json({ error: playerErr?.message ?? "Erreur joueur" }, { status: 500 });
    }

    await admin.from("retro_rooms").update({ host_player_id: player.id }).eq("id", room.id);
    await admin.from("usage").insert({ user_id: user.id, tool: "retrospective" });
    await markWeeklyCoachComplete(user.id, "retrospective");
