...
export async function POST(
  ...
  if (room.status !== "writing") return NextResponse.json({ error: "Impossible de révéler depuis cet état" }, { status: 409 });
  if (room.host_player_id !== playerId) return NextResponse.json({ error: "Non autorisé" }, { status: 403 });

  const isValid = await verifyPlayerSecret(admin, "kudo_players", playerId, playerSecret);
  if (!isValid) return NextResponse.json({ error: "Non autorisé" }, { status: 403 });

  await admin.from("kudo_rooms").update({ status: "revealed" }).eq("id", room.id);
