...
export async function POST(req: NextRequest) {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    if (playerErr || !player) {
      await admin.from("humeur_rooms").delete().eq("id", room.id);
    ...
    if (creatorUserId) {
      await admin.from("usage").insert({ user_id: creatorUserId, tool: "humeur" });
      await markWeeklyCoachComplete(creatorUserId, "humeur");
    }
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    console.error("[DELETE /api/humeur/room]", err);
