...
export async function POST(req: NextRequest) {
  ...
  const { pseudo, avatarColor, questionTheme = "all", questionCount = 10 } = body;
  ...
  for (let i = 0; i < 5; i++) {
    ...
    if (playerErr || !player) {
      await admin.from("tribu_rooms").delete().eq("id", room.id);
    ...
    if (creatorUserId) await admin.from("usage").insert({ user_id: creatorUserId, tool: "tribu" });
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    console.error("[DELETE /api/tribu/room]", err);
