...
import { getRandomChallenge } from "@/lib/code-secret/challenges";
import type { Difficulty, GameMode } from "@/lib/code-secret/types";
...
export async function GET() {
  ...
  const { data: rooms } = await admin
    ...
    .select("id, code, status, game_mode, difficulty, challenge_id, created_at")
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { pseudo, avatarColor, gameMode, difficulty, team } = body;
    if (!pseudo?.trim() || !gameMode || !difficulty) {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const { data: player, error: playerErr } = await admin
        ...
        .insert({
          ...
          is_host: true,
          team: gameMode === "competitive" ? (team ?? null) : null,
      ...
      if (playerErr || !player) {
        await admin.from("code_secret_rooms").delete().eq("id", room.id);
        return NextResponse.json({ error: playerErr?.message ?? "Erreur joueur" }, { status: 500 });
      }

      await admin.from("code_secret_rooms").update({ host_player_id: player.id }).eq("id", room.id);
      await admin.from("usage").insert({ user_id: user.id, tool: "code-secret" });
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    console.error("[DELETE /api/code-secret/room]", err);
    return NextResponse.json({ error: "Erreur serveur" }, { status: 500 });
  }
}
