...
export default function ChaineLobbyPage() {
  ...
  const shareUrl =
    typeof window !== "undefined"
      ? `${window.location.origin}/toolbox/chaine/join?code=${upperCode}`
      : "";
  ...
  useEffect(() => {
    ...
    if (!stored) {
      router.replace(`/toolbox/chaine/join?code=${upperCode}`);
      return;
    }
    try {
  ...
  useEffect(() => {
    ...
    async function fetchRoom() {
      ...
      const data = await res.json() as ChaineRoom & { players: ChainePlayer[] };
      setRoom(data);
      setPlayers(data.players ?? []);
      setLoading(false);

      if (data.status === "playing") {
        router.push(`/toolbox/chaine/${upperCode}/play`);
      } else if (data.status === "voting" || data.status === "finished") {
        router.push(`/toolbox/chaine/${upperCode}/results`);
      }
    }
  ...
  async function startGame() {
    ...
    setStartError(null);
    try {
      const res = await fetch(`/api/chaine/room/${upperCode}/start`, {
        method: "POST",
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 flex flex-col">
      ...
      <main className="flex-1 max-w-sm mx-auto w-full px-6 py-10">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-5xl font-bold font-mono tracking-widest text-gray-900 dark:text-white mb-1">
            {upperCode}
          </p>
          <p className="text-sm text-violet-600 dark:text-violet-400 font-medium mb-4">
            Mot de départ : <span className="font-bold">{room.starter_word}</span>
          </p>

          <QRShare
            url={shareUrl}
            label="Partagez ce lien aux joueurs"
            filename={`chaine-${upperCode}`}
        ...
        {/* Double-tour info badge */}
        {players.length >= 2 && players.length <= 3 && (
          <div className="flex items-center justify-center gap-1.5 text-xs text-violet-600 dark:text-violet-400 mb-4">
            <span>↩</span>
            <span>
              Avec {players.length} joueurs, chaque joueur jouera <strong>2 fois</strong>.
            </span>
          </div>
        )}
