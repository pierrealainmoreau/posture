...
import { Link2, Plus, Hash, ChevronRight, Clock, Users, CheckCircle2, Loader2, Trash2, X } from "lucide-react";
...
function RoomCard({ room, onDelete }: { room: RoomRow; onDelete: (id: string) => void }) {
  ...
  async function handleDelete(e: React.MouseEvent) {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`/api/chaine/room?code=${room.code}`, { method: "DELETE" });
  ...
  return (
    <div className="group flex items-center bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl hover:border-violet-300 dark:hover:border-violet-700 hover:shadow-sm transition-all">
      <Link href={`/toolbox/chaine/${room.code}/lobby`} className="flex-1 flex items-center gap-4 px-5 py-4 min-w-0">
