...
export default function ThisOrThatLobbyPage() {
  ...
  const [room, setRoom]               = useState<ThisOrThatRoom | null>(null);
  ...
  const shareUrl =
    typeof window !== "undefined"
      ? `${window.location.origin}/toolbox/thisorthat/join?code=${upperCode}`
      : "";

  useEffect(() => {
    const stored = localStorage.getItem(`thisorthat_player_${upperCode}`);
    if (!stored) { router.replace(`/toolbox/thisorthat/join?code=${upperCode}`); return; }
    try {
      const { playerId: pid, playerSecret: secret } = JSON.parse(stored) as { playerId: string; playerSecret?: string };
      setPlayerId(pid);
      setPlayerSecret(secret ?? "");
    } catch {
      router.replace(`/toolbox/thisorthat/join?code=${upperCode}`);
    }
  ...
  useEffect(() => {
    ...
    async function fetchRoom() {
      ...
      const data = await res.json() as ThisOrThatRoom & { players: ThisOrThatPlayer[] };
      ...
      if (data.status === "playing") {
        router.push(`/toolbox/thisorthat/${upperCode}/play`);
      } else if (data.status === "finished") {
        router.push(`/toolbox/thisorthat/${upperCode}/results`);
      }
    }
  ...
  async function startGame() {
    ...
    setStartError(null);
    try {
      const res = await fetch(`/api/thisorthat/room/${upperCode}/start`, {
        method: "POST",
      ...
      <main className="flex-1 max-w-sm mx-auto w-full px-6 py-10">
        ...
        <div className="text-center mb-8">
          ...
          <QRShare
            ...
            filename={`thisorthat-${upperCode}`}
            wrapperCls="bg-sky-50 dark:bg-sky-950/40 border-sky-100 dark:border-sky-900"
            labelCls="text-sky-700 dark:text-sky-300"
            urlCls="text-sky-800 dark:text-sky-200"
            copyBtnCls="bg-white dark:bg-sky-900 border-sky-200 dark:border-sky-700 text-sky-700 dark:text-sky-300 hover:bg-sky-50 dark:hover:bg-sky-800"
          />
        </div>
        ...
        {isHost && (
          <div className="flex flex-col items-stretch gap-2">
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 py-3 bg-sky-500 text-white text-sm font-semibold rounded-xl hover:bg-sky-600 disabled:opacity-40 transition-colors"
            >
