...
export default function UndercoverLobbyPage() {
  ...
  const shareUrl =
    typeof window !== "undefined"
      ? `${window.location.origin}/toolbox/undercover/join?code=${upperCode}`
      : "";
  ...
  const fetchRoom = useCallback(async () => {
    ...
    if (data.status === "description" || data.status === "discussion" || data.status === "voting" || data.status === "mr_white_guess") {
      router.push(`/toolbox/undercover/${upperCode}/play`);
    } else if (data.status === "finished") {
      router.push(`/toolbox/undercover/${upperCode}/results`);
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
