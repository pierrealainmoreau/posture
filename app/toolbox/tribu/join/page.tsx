...
function JoinForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(`/api/tribu/room/${upperCode}/players`, {
        method: "POST",
      ...
      // Vider toute session stale, puis stocker le nouvel id (identique à Draw It)
      localStorage.removeItem(`tribu_player_${upperCode}`);
      localStorage.removeItem(`tribu_pending_${upperCode}`);
      localStorage.setItem(
        `tribu_player_${upperCode}`,
        JSON.stringify({ playerId: data.playerId, playerSecret: data.playerSecret, pseudo: pseudo.trim(), avatarColor })
      );

      router.push(`/toolbox/tribu/${upperCode}/lobby`);
    } catch {
...
export default function TribuJoinPage() {
  return (
    <Suspense>
      <JoinForm />
    </Suspense>
