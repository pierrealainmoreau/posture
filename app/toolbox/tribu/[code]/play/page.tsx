...
export default function TribuPlayPage() {
  ...
  useEffect(() => {
    ...
    if (!stored) { router.replace(`/toolbox/tribu/join?code=${upperCode}`); return; }
    try {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      if (roomData.status === "lobby") {
        router.replace(`/toolbox/tribu/${upperCode}/lobby`);
        return;
      }
      if (roomData.status === "revealing" || roomData.status === "finished") {
        router.replace(`/toolbox/tribu/${upperCode}/results`);
        return;
      }
