...
import { Compass, Plus, Hash, Clock, Users, CheckCircle2, Loader2, Trash2, X } from "lucide-react";
...
function RoomCard({ room, onDelete }: { room: RoomRow; onDelete: (id: string) => void }) {
  ...
  async function handleDelete(e: React.MouseEvent) {
    ...
    setDeleting(true);
    try {
      await fetch(`/api/boussole/room?code=${room.code}`, { method: "DELETE" });
      localStorage.removeItem(`boussole_player_${room.code}`);
      onDelete(room.id);
  ...
  return (
    <div className="group flex items-center bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl hover:border-indigo-300 dark:hover:border-indigo-700 hover:shadow-sm transition-all">
      <Link
        href={isActive ? `/toolbox/boussole/${room.code}/lobby` : `/toolbox/boussole/${room.code}/results`}
