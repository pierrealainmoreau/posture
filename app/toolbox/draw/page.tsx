...
function RoomCard({ room, onDelete }: { room: RoomRow; onDelete: (id: string) => void }) {
  ...
  async function handleDelete(e: React.MouseEvent) {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`/api/draw/room?code=${room.code}`, { method: "DELETE" });
  ...
  return (
    <div className="group flex items-center bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl hover:border-orange-300 dark:hover:border-orange-700 hover:shadow-sm transition-all">
      <Link
        href={isActive ? `/toolbox/draw/${room.code}/lobby` : `/toolbox/draw/${room.code}/history`}
        className="flex-1 flex items-center gap-4 px-5 py-4 min-w-0"
      >
        <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${
          isActive ? "bg-orange-50 dark:bg-orange-950" : "bg-gray-100 dark:bg-gray-800"
        }`}>
          {isActive
