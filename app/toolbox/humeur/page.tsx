...
import {
  ...
  CheckCircle2, Loader2, Zap, Trash2, X,
} from "lucide-react";
...
function RoomCard({ room, onDelete }: { room: RoomRow; onDelete: (id: string) => void }) {
  ...
  const linkPath = isActive
    ? `/toolbox/humeur/${room.code}/lobby`
    : `/toolbox/humeur/${room.code}/results`;
  ...
  async function handleDelete(e: React.MouseEvent) {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`/api/humeur/room?code=${room.code}`, { method: "DELETE" });
  ...
  return (
    <div className="group flex items-center bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl hover:border-rose-300 dark:hover:border-rose-700 hover:shadow-sm transition-all">
      <Link href={linkPath} className="flex-1 flex items-center gap-4 px-5 py-4 min-w-0">
...
export default function HumeurPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 flex flex-col">
      <Header backHref="/" parentHref="/toolbox" parentLabel={t.common.miniJeux} currentTool={t.humeur.title} />
