...
export default function AbcdePlayPage() {
  ...
  useEffect(() => {
    ...
    if (!stored) { router.replace(`/toolbox/abcde/join?code=${upperCode}`); return; }
    try {
      const { playerId: pid } = JSON.parse(stored) as { playerId: string };
      setPlayerId(pid);
    } catch {
      router.replace(`/toolbox/abcde/join?code=${upperCode}`);
    }
  ...
  const fetchRoom = useCallback(async () => {
    ...
    if (data.status === "lobby") router.push(`/toolbox/abcde/${upperCode}/lobby`);
    if (data.status === "finished") router.push(`/toolbox/abcde/${upperCode}/results`);
  ...
  async function synthesize() {
    ...
    else router.push(`/toolbox/abcde/${upperCode}/results`);
