...
export default function DrawLobbyPage() {
  ...
  const shareUrl =
    typeof window !== "undefined"
      ? `${window.location.origin}/mini-jeux/draw/join?code=${upperCode}`
      : "";
  ...
  useEffect(() => {
    ...
    if (!stored) {
      router.replace(`/mini-jeux/draw/join?code=${upperCode}`);
      return;
    }
    try {
  ...
  useEffect(() => {
    ...
    async function fetchRoom() {
      ...
      if (data.status === "playing") {
        router.push(`/mini-jeux/draw/${upperCode}/play`);
      } else if (data.status === "finished") {
        router.push(`/mini-jeux/draw/${upperCode}/results`);
      }
    }
