...
import {
  PenLine, Plus, Hash, ChevronRight, Clock, Users,
...
function RoomCard({ room }: { room: RoomRow }) {
  ...
  const roundsPlayed = room.status === "finished"
    ? room.rounds_total
    : Math.max(0, room.current_round - 1);

  return (
    <Link
      href={isActive
        ? `/mini-jeux/draw/${room.code}/lobby`
        : `/mini-jeux/draw/${room.code}/history`}
      className="group flex items-center justify-between bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl px-5 py-4 hover:border-orange-300 dark:hover:border-orange-700 hover:shadow-sm transition-all"
    >
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-xs text-gray-400 dark:text-gray-500 flex items-center gap-2">
            ...
            <span className="text-gray-300 dark:text-gray-600">·</span>
            {roundsPlayed}/{room.rounds_total} {room.rounds_total !== 1 ? t.common.rounds : t.common.round}
          </p>
        </div>
      </div>

      <ChevronRight size={15} className="text-gray-300 dark:text-gray-600 group-hover:text-orange-500 transition-colors flex-shrink-0" />
    </Link>
  );
}
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 flex flex-col">
      ...
      <main className="flex-1 max-w-2xl mx-auto w-full px-6 py-10">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-xl font-semibold text-gray-900 dark:text-white">Draw It</h1>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            <div className="w-14 h-14 rounded-2xl bg-orange-50 dark:bg-orange-950 flex items-center justify-center mb-4">
              <PenLine size={24} className="text-orange-600 dark:text-orange-400" />
            </div>
