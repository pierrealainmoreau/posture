...
export default function SpeedRetroVotePage() {
  ...
  useEffect(() => {
    ...
    if (!stored) { router.replace(`/mini-jeux/speed-retro/join?code=${upperCode}`); return; }
    try {
      const { playerId: pid, playerSecret: secret } = JSON.parse(stored) as { playerId: string; playerSecret?: string };
      setPlayerId(pid);
      setPlayerSecret(secret ?? "");
    } catch {
      router.replace(`/mini-jeux/speed-retro/join?code=${upperCode}`);
    }
  ...
  useEffect(() => {
    ...
    async function fetchRoom() {
      ...
      if (data.status === "finished") router.push(`/mini-jeux/speed-retro/${upperCode}/results`);
      else if (data.status === "writing") router.push(`/mini-jeux/speed-retro/${upperCode}/write`);
      else if (data.status === "lobby") router.push(`/mini-jeux/speed-retro/${upperCode}/lobby`);
    }
