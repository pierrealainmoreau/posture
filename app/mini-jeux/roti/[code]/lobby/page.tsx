...
export default function RotiLobbyPage() {
  ...
  useEffect(() => {
    ...
    if (!stored) {
      router.replace(`/mini-jeux/roti/join?code=${upperCode}`);
      return;
    }
    try {
  ...
  useEffect(() => {
    ...
    async function fetchRoom() {
      ...
      if (data.status === "voting") router.push(`/mini-jeux/roti/${upperCode}/vote`);
      else if (data.status === "finished") router.push(`/mini-jeux/roti/${upperCode}/results`);
    }
