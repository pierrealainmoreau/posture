...
function JoinForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(`/api/boussole/room/${upperCode}/players`, {
        method: "POST",
      ...
      if (!res.ok) {
        ...
        else if (res.status === 409) setError(t.gameSetup.errorAlreadyStarted);
          ...
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-5 space-y-4">
            ...
            <div>
              ...
              <input
                ...
                placeholder="ABC123"
                maxLength={6}
                className="w-full px-3 py-2.5 text-sm font-mono tracking-widest text-center uppercase rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-900 dark:text-white placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500"
              />
            </div>
...
export default function BoussolerJoinPage() {
  return (
    <Suspense>
      <JoinForm />
    </Suspense>
