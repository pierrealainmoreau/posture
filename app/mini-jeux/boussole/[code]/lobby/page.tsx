...
export default function BoussoleLobbyPage() {
  const { t } = useI18n();
  ...
  const [room, setRoom] = useState<BoussolerRoom | null>(null);
  const [players, setPlayers] = useState<BoussolerPlayer[]>([]);
  ...
  const shareUrl =
    typeof window !== "undefined"
      ? `${window.location.origin}/boussole/join?code=${upperCode}`
      : "";

  const isHost = room?.host_player_id === playerId;

  const breadcrumbs = isHost
    ? [{ href: "/boussole", label: t.boussole.title }, { label: upperCode }]
    : undefined;

  // Load player identity
  useEffect(() => {
    const stored = localStorage.getItem(`boussole_player_${upperCode}`);
    if (!stored) {
      router.replace(`/mini-jeux/boussole/join?code=${upperCode}`);
      return;
    }
    try {
  ...
  useEffect(() => {
    ...
    async function fetchRoom() {
      const res = await fetch(`/api/boussole/room/${upperCode}`);
      if (!res.ok) { setLoading(false); return; }
      const data = await res.json() as BoussolerRoom & { players: BoussolerPlayer[] };
      setRoom(data);
      setPlayers(data.players ?? []);
      setLoading(false);

      if (data.status === "playing") {
        router.push(`/mini-jeux/boussole/${upperCode}/play`);
      } else if (data.status === "finished") {
        router.push(`/mini-jeux/boussole/${upperCode}/results`);
      }
    }
  ...
  async function startGame() {
    ...
    setStartError(null);
    try {
      const res = await fetch(`/api/boussole/room/${upperCode}/start`, {
        method: "POST",
