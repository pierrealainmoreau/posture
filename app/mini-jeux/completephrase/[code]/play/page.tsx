...
export default function CompletePhrasePlayPage() {
  ...
  const fetchRoom = useCallback(async () => {
    ...
    if (data.status === "finished") { router.push(`/mini-jeux/completephrase/${upperCode}/results`); return; }
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      const data = await res.json() as RoomData;
      if (data.status === "lobby") { router.replace(`/mini-jeux/completephrase/${upperCode}/lobby`); return; }
      if (data.status === "finished") { router.replace(`/mini-jeux/completephrase/${upperCode}/results`); return; }
