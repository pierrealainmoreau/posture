...
export default function CodeSecretLobbyPage() {
  ...
  const shareUrl = typeof window !== "undefined"
    ? `${window.location.origin}/mini-jeux/code-secret/join?code=${upperCode}`
    : "";
  ...
  useEffect(() => {
    ...
    if (!stored) { router.replace(`/mini-jeux/code-secret/join?code=${upperCode}`); return; }
    try {
      const { playerId: pid, playerSecret: secret } = JSON.parse(stored) as { playerId: string; playerSecret?: string };
      setPlayerId(pid);
      setPlayerSecret(secret ?? "");
    } catch {
      router.replace(`/mini-jeux/code-secret/join?code=${upperCode}`);
    }
  ...
  useEffect(() => {
    ...
    async function fetchRoom() {
      ...
      setRoom(data);
      setLoading(false);
      if (data.status === "playing") router.push(`/mini-jeux/code-secret/${upperCode}/play`);
      else if (data.status === "finished") router.push(`/mini-jeux/code-secret/${upperCode}/results`);
    }
    fetchRoom();
