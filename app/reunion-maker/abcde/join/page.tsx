...
function JoinForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(`/api/abcde/room/${upperCode}/players`, {
        method: "POST",
      ...
      localStorage.setItem(`abcde_player_${upperCode}`, JSON.stringify({ playerId: data.playerId, pseudo: pseudo.trim(), avatarColor }));
      router.push(`/reunion-maker/abcde/${upperCode}/lobby`);
    } catch {
      ...
      <main className="flex-1 max-w-sm mx-auto w-full px-6 py-10">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-5 space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Code de la session</label>
              <input
                type="text"
                required
                autoFocus
...
export default function AbcdeJoinPage() {
  return (
    <Suspense>
      <JoinForm />
    </Suspense>
