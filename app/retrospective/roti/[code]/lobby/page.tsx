...
export default function RotiLobbyPage() {
  ...
  useEffect(() => {
    ...
    if (!stored) {
      router.replace(`/retrospective/roti/join?code=${upperCode}`);
      return;
    }
    try {
  ...
  useEffect(() => {
    ...
    async function fetchRoom() {
      ...
      if (data.status === "voting") router.push(`/retrospective/roti/${upperCode}/vote`);
      else if (data.status === "finished") router.push(`/retrospective/roti/${upperCode}/results`);
    }
